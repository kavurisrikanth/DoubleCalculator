...
public class MeasuredValueRecognizer {
	...
	private static String displayInput(String input, int currentCharNdx) {
		/*
		 * Commented out to avoid printing this multiple times.
	...
	public static String checkMeasureValue(String input) {
		...
		while (running) {
			...
			case 5: 
				// State 5 has 2 valid transitions.
				
				// Not a final state.
				finalState = false;
				
				// The current character may be a digit, or '+', or '-'. Each case is handled below.
				if(currentChar >= '0' && currentChar <= '9') {
					// A digit. Move to state 7.
					nextState = 7;
				} else if(currentChar == '+' || currentChar == '-') {
					// + or -. Move to state 6.
					nextState = 6;
				} else {
					// Invalid input. We stop.
					running = false;
				}
			...
			case 6: 
				// State 6 has only one valid transaction. We got here via a '+' or a '-'. We only move on if we get a digit.
				
				// Not a final state.
				finalState = false;
				
				// The current character may only be a digit.
				if(currentChar >= '0' && currentChar <= '9') {
					// A digit. Move to state 7.
					nextState = 7;
				} else {
			...
			case 7: 
				// State 7 has only one valid transaction. Anything other than a digit is invalid input.
